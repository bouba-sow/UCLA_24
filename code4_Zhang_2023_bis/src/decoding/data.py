"""Data loading — Zhang protocol, leakage-free evaluation (code4)."""
...
from constants import DNK_MARGIN, FRAME_SUBSAMPLE, HALF_WINDOW_FRAMES, UNIT_RATE_MIN_HZ
...
def apply_dnk_full_timeline(raw: np.ndarray, dnk_margin: int = DNK_MARGIN) -> np.ndarray:
    """DNK on full 30 Hz timeline (±dnk_margin frames), then subsample."""
    labels = raw.copy()
    n = raw.shape[0]
...
def load_firing_rates(
    ...
    min_rate_hz: float = UNIT_RATE_MIN_HZ,
...
class CharacterDataset(Dataset):
    ...
    def __len__(self) -> int:
        return len(self.sample_i)

    def __getitem__(self, i: int) -> tuple[torch.Tensor, torch.Tensor]:
        si = int(self.sample_i[i])
        f = int(self.sample_frames[i])
        hw = self.half_win
        x = torch.from_numpy(self.rates[f - hw : f + hw].copy())
        y = torch.from_numpy(self.labels[si].astype(np.int64))
        return x, y


def fit_normalizer(rates: np.ndarray, center_frames: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    subset = rates[center_frames]
